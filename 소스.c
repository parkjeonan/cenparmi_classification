		...
		for (int i = 1; i <= iter; i++) {
			...
			for (int batch = 0; batch < 100; batch++) {
				...
				for (int m = 0; m < 21; m++) {
					...
					deriv_V[batch][m] = deriv_W[batch][m] * (1 - Z_sigmoid[batch][m]) * Z_sigmoid[batch][m];
					//printf("%lf ", deriv_V[batch][m]);
					for (int q = 0; q < 101; q++) {
						aver_deriv_Z[q][m] += deriv_V[batch][m] * batch_x[batch][q];
						//printf("%lf ", aver_deriv_Z[q][m]);
					}
					//printf("\n");
				}
				//printf("\n");
				...
				if (batch == 99) {
					for (int q = 0; q < 21; q++) {
						for (int m = 0; m < 10; m++) {
							aver_deriv_W[q][m] = (double)aver_deriv_W[q][m] / 100;
						}
					...
					for (int q = 0; q < 101; q++) {
						for (int m = 0; m < 21; m++) {
							aver_deriv_Z[q][m] = (double)aver_deriv_Z[q][m] / 100;
					...
					for (int p = 0; p < 21; p++) {
						for (int k = 0; k < 10; k++) {
							W[p][k] += ALPHA * aver_deriv_W[p][k];
						}
					}

					for (int n = 0; n < 101; n++) {
						for (int m = 0; m < 21; m++) {
							V[n][m] += ALPHA * aver_deriv_Z[n][m];
